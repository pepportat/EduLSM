...
public class SsTableRenderer
{
    ...
    public List<SearchResult> SsTablesSearchResults { get; set; }
    ...
    public Dictionary<int, List<SsTable>> SsTables { get; set; }
    public SsTableTabState SsTableTabState;
    public List<int> CompactionNeededTiers { get; set; }
    private int? CleanupNeededTier { get; set; }
    private CompactionVisualizerData? _compactionVisualizerData;
    private readonly string _dataPath;
    private readonly UIState _uiState;

    public SsTableRenderer(string dataPath, UIState uiState)
    {
        _dataPath = dataPath;
        _uiState = uiState;
        SsTablesSearchResults = [];
        SsTables = ReadAllTables(dataPath);
        CompactionNeededTiers = NeedsCompaction(dataPath);
        CleanupNeededTier = null;
        SsTableTabState = CompactionNeededTiers.Count == 0 ? SsTableTabState.Viewing : SsTableTabState.Compacting;
    }
    ...
    private void DrawCompactButton()
    {
        var mouseCursor = Button.DrawActionButton(
            ...
            () =>
            {
                SsTableTabState = SsTableTabState.Compacting;
    ...
    private void DrawFileCleanupButton()
    {
        var mouseCursor = Button.DrawActionButton(
            ...
            () =>
            {
                ...
                {
                    SsTableTabState = SsTableTabState.Viewing;
                }
            }
        );
    ...
    {
        DrawTextEx(Font, _dataPath, new Vector2(10, _uiState.ScreenHeight - FontSize - 10), FontSize, 2, Color.White);
    }
}